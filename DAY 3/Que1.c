...
void insert_any(int data)
        {
    	struct node *temp = (struct node*)malloc(sizeof(struct node));  //it will insert_anyert a new node to the start of the linked list
    	temp->data=data;
    	temp->next=head;
    	head=temp;
        }

void insert_any_at_pos_n(int data,int position)
    {
